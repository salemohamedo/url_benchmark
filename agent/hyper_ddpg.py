...
from agent.hyper_utils import apply_sn, PoincarePlaneDistance, ClipNorm, final_weight_init_hyp_small
...
class Actor(nn.Module):
    def __init__(self, obs_type, obs_dim, action_dim, feature_dim, hidden_dim):
        ...
        final_weight_init_hyp_small(self.policy[-1])
...
class Critic(nn.Module):
    def __init__(self, obs_type, obs_dim, action_dim, feature_dim, hidden_dim):
        ...
        self.apply(utils.weight_init)
        final_weight_init_hyp_small(self.Q1[-1])
        final_weight_init_hyp_small(self.Q2[-1])
...
class HyperDDPGAgent(nn.Module):
    def __init__(self,
                 name,
                 reward_free,
                 obs_type,
        ...
        if obs_type == 'pixels':
            self.encoder_opt = RiemannianAdam(self.encoder.parameters(),
                                                lr=lr)
        else:
            self.encoder_opt = None
        self.actor_opt = RiemannianAdam(self.actor.parameters(), lr=lr)
        self.critic_opt = RiemannianAdam(self.critic.parameters(), lr=lr)
